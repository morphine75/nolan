...
function guardar($conn){
	...
	if ($id==0){
		$sql="INSERT INTO almacenes (DESALMA, ID_DEPOSITO) VALUES ('".$nombre."','".$deposito."')";
...
function eliminar($conn){
	$id=$_REQUEST['id'];
	$sql="DELETE FROM almacenes where ID_DEPOSITO=".$id;
	$res=@mysqli_query($conn,$sql);
	if ($res === false) {
		$sql="UPDATE ALMACENES SET ANULADO=1 WHERE ID_ALMACEN=".$id;
		$res=mysqli_query($conn,$sql);
