...
?>
  <div> 
   ...
   if($_REQUEST['id']>0){
    ...
    $id=$_REQUEST['id'];
    $sql=mysqli_query($conn,"SELECT P.ID_PEDIDO, P.ID_CLIENTE, P.ID_VENDEDOR, C.NOM_CLIENTE, V.NOM_VENDEDOR, P.FECHA, P.HORA, P.TOTAL from pedidos P, clientes C, vendedores V where P.ID_CLIENTE=C.ID_CLIENTE AND P.ID_VENDEDOR=V.ID_VENDEDOR AND ID_PEDIDO=".$id);
    ...
    <form method="post" id="formulario">
      ...
      <div id="detalle">
        <table class="table table-striped">
            ...
            if ($id>0){
              while ($rowD=mysqli_fetch_assoc($resD)){
                ...
                <tr id="tr_<?php echo $i?>">
                  <td><input type="text" class="inactivo" readonly="readonly" size="7" name="codigo[]" id="codigo<?php echo $i?>" value="<?php echo $rowD['ID_ARTICULO']?>"></td>
                  <td><input type="text"  name="articulo[]" size="30" id="articulo" onKeyUp="busca_art_codigo(this.value,<?php echo $i?>)" value="<?php echo $rowD['DESCRIPCION']?>"></td>
                  <td><span id="precio<?php echo $i?>"><?php echo $rowD['P_UNITARIO']; ?></span></td>
                  <td><input type="number" min="0" step="1" name="cantidad[]" size="5" id="cantidad<?php echo $i?>" onkeyup="calcula_st(this.value,0,<?php echo $i?>)" value="<?php echo floor($rowD['CANT']/$rowD['CANTXCAJA'])?>"></td>
                  <td><input type="number" min="0" step="1" name="unidades[]" size="5" id="unidades<?php echo $i?>" onkeyup="calcula_st(this.value,1,<?php echo $i?>)" value="<?php echo ($rowD['CANT']%$rowD['CANTXCAJA'])?>"></td>
                  ...
                  <td><span id="subtotal<?php echo $i?>"><?php echo $rowD['PRECIO']?></span></td></tr>
         ...
         <div class="col-md-12" style="box-shadow: 10px 10px 5px grey; background-color: #e6f2ff;">
        <div class="col-md-12" id="lista_articulos"></div>
        </div>
      </div>
    </form>
