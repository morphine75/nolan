...
function guardar($conn){
	...
	if ($id==0){
		$sql="INSERT INTO RUTAS (DESCRIPCION, ID_SUCURSAL, ID_TIPO_RUTA, ANULADO) VALUES ('".$nombre."', '".$sucursal."','1', 0)";
...
function eliminar($conn){
	...
	if ($res === false) {
		...
		$res=mysqli_query($conn,$sql);
		echo '<div class="alert alert-danger alert-dismissable"> <button type="button" class="close" data-dismiss="alert">&times;</button> <i class="glyphicon glyphicon-ok-sign"></i> <strong>La ruta se encuentra relacionada a clientes, se procedio a anularla </strong></div>';
	}
	else{
