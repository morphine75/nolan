...
function guardar($conn){
	...
	if ($id==0){
		$sql="INSERT INTO sucursales (DESCRIPCION) VALUES ('".$nombre."')";
		...
		$sql="UPDATE sucursales SET DESCRIPCION='".$nombre."' WHERE ID_SUCURSAL=".$id;
		$res=@mysqli_query($conn,$sql);
		if ($res === false) {
...
function eliminar($conn){
	...
	$res=@mysqli_query($conn,$sql);
	if ($res === false) {
		$sql="UPDATE sucursales SET ANULADO=1 WHERE ID_SUCURSAL=".$id;
		$res=mysqli_query($conn,$sql);
		echo '<div class="alert alert-danger alert-dismissable"> <button type="button" class="close" data-dismiss="alert">&times;</button> <i class="glyphicon glyphicon-ok-sign"></i> <strong>La sucursal se encuentra relacionada a vendedores, se procedio a anularlo </strong></div>';
	}
	else{
